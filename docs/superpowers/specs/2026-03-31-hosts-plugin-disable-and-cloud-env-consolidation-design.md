...
**Note:** This is the initial hosts plugin PR — no released VHD or CSE has hosts plugin code yet. The scenarios below are forward-looking design rationale for *future* changes to the environment file or service unit after this PR ships. They are not testable against the current main branch.

- **Future: new CSE on old VHD (after a subsequent service file change):** `disableAKSHostsSetup()` is in `cse_config.sh` which is part of CSE, not VHD — CSE carries its own functions. If a future change modifies `aks-hosts-setup.service`, old VHDs would still have the previous version. The `-` prefix on `EnvironmentFile` makes the file non-fatal when absent, and `aks-hosts-setup.sh` exits gracefully on empty `LOCALDNS_CRITICAL_FQDNS` (line 23-26).
