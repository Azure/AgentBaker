...
// vhdbuilder/release-notes/AKSAIManager/<model-slug>/<version>.json
//
// One file per model, date-versioned. Each file declares every (gpu, os) variant
...
//
// Validate with:
//   cue vet -c ./schemas/aimanager.cue ./vhdbuilder/release-notes/AKSAIManager/<model>/<version>.json
...
#Model: {
	...
	weightsSizeGiB:    number & >0 // approx on-disk size of the baked weights; the consumer derives the OS/pool disk size from this (weights + base VHD + CUDA + headroom)
	bakePath:          string // weights bake dir; must be KAITO's location /opt/kaito/models/<hfRepo lowercased, '/' and '\' replaced by '-'>
	expectedFileCount?: int   // optional fail-closed gate on the baked weight file count
}
...
#Dependency: {
	...
	version:         string // version identifier / pin, e.g. "12.9"
	...
	sourcePath?:     string // optional: package install location, if it must be relocated to bakePath
}

// The vanilla AKS VHD lineage a variant overlays.
#BaseImage: {
	offer:   string // e.g. "AKSUbuntu"
	sku:     string // e.g. "2404gen2containerd"
	version: string // pinned source version, e.g. "202606.19.0"
}
