...
if [[ -n "${SUBSCRIPTION_ID}" ]]; then
 ...
 # So we will need to rely on the "tags.createdtime" property in the resource group metadata to determine the creation date.
  ...
  for rg_group in $rg_groups; do
    lock=$(az lock list --resource-group $rg_group --query "contains([].level, 'CanNotDelete')" --output tsv)
    # loop resource groups and only delete resource groups that are not locked
    if [[ $lock = true ]]; then
      echo "The resource group named ${rg_group} has a CanNotDelete lock."
    else
