...
# Create a temporary directory for the CSE package
releaseTempDir=$(mktemp -d)
stagingDir=$(mkdir "$releaseTempDir/windows")


echo "Temporary directory created: $releaseTempDir" 
# Copy the CSE package files to the temporary directory
cp -r staging/cse/windows/* "$stagingDir"
# Remove unnecessary files from the CSE package
echo "Removing unnecessary files from the CSE package..."
...
echo "Creating zip file: $csePackage"
echo "Creating release pacakge $releaseTempDir/$csePackage"
pushd $stagingDir
...
echo "Cleaning up temporary folder $releaseTempDir."
