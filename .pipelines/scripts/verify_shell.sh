...
filesToCheck=$(find . -type f -name "*.sh" -not -path './parts/linux/cloud-init/artifacts/*' -not -path './pkg/agent/testdata/*' -not -path './vendor/*'  -not -path './bash_tests/*' -not -path './hack/tools/vendor/*' -not -path './.git/*' -not -path './self-contained/*')
