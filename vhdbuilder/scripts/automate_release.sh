...
create_release() {
    ...
    RESPONSE=$(curl --http1.1 -X POST -H "User-Agent: vhdbuilder/1.0" -H "Authorization: Basic $(echo -n ":$ADO_PAT" | base64)" -H "Content-Type: application/json" -d "$REQUEST_BODY" "$RELEASE_API_ENDPOINT")
