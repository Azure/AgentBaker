#!/usr/bin/env bash
set -e
(>&2 echo "Remediating rule 915/364: 'audit_rules_login_events_faillock'")
# Remediation is applicable only in certain platforms
if rpm --quiet -q audit; then

if selinuxenabled; then
    #  Manually add this requirement so we can make sure we have semanage present on the image
    if ! rpm -q --quiet "policycoreutils-python-utils" ; then
        dnf install -y "policycoreutils-python-utils"
    fi

    # altering security context for /var/log/faillock because policy expects it to be
    # in /run/faillock causing SELinux denials. 
