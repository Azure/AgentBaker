#!/usr/bin/env bash
set -e
(>&2 echo "Remediating rule 63/364: 'accounts_passwords_pam_faillock_deny'")
...
# auth required pam_faillock.so preauth is not present, insert the whole line
        ...
        control="\[success=1 default=bad\]"
        sed -i --follow-symlinks "/^auth.*$control.*pam_unix.so.*/i auth        required      pam_faillock.so preauth silent ""deny"'='"$var_accounts_passwords_pam_faillock_deny" "$pam_file"
fi
...
# auth default pam_faillock.so authfail is not present, insert the whole line
        ...
        control="\[success=1 default=bad\]"
        sed -i --follow-symlinks "/^auth.*$control.*pam_unix.so.*/a auth        [default=die] pam_faillock.so authfail ""deny"'='"$var_accounts_passwords_pam_faillock_deny" "$pam_file"
fi
