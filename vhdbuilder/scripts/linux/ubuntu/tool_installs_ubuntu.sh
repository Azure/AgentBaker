...
installAscBaseline() {
   ...
   ASC_BASELINE_TMP=asc-baseline-1.0.0.35.amd64.deb
...
installBcc() {
    ...
    if [[ "${VERSION}" == "22.04" ]]; then
        apt_get_install 120 5 300 build-essential git bison cmake flex libedit-dev libllvm14 llvm-14-dev libclang-14-dev zlib1g-dev libelf-dev python3-distutils libfl-dev || exit $ERR_BCC_INSTALL_TIMEOUT
    else
