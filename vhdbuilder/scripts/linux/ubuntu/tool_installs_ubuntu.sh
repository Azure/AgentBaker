...
installBcc() {
    ...
    if [ "${VERSION}" = "26.04" ]; then
        apt_get_purge 120 5 300 bison cmake flex libedit-dev libllvm18 llvm-18-dev libclang-18-dev zlib1g-dev libelf-dev libfl-dev || exit $ERR_BCC_INSTALL_TIMEOUT
    elif [ "${VERSION}" = "22.04" ] || [ "${VERSION}" = "24.04" ]; then
