...
installBcc() {
    ...
    if [ "${VERSION}" = "22.04" ] || [ "${VERSION}" = "24.04" ]; then
        ...
        # LLVM is pinned to 18 (not 26.04's default 22): BCC v0.37.0 links the individual clang
        ...
        # 18 is the newest major with an explicit clang-lib guard in BCC's CMakeLists and is in the 26.04 archive.
        apt_get_install 120 5 300 build-essential git bison cmake flex libedit-dev libllvm18 llvm-18-dev libclang-18-dev python3 zlib1g-dev libelf-dev libfl-dev || exit $ERR_BCC_INSTALL_TIMEOUT
    else
    ...
    # libPolly.a is needed for the make target that runs later, which is not present in the default patch version of llvm-18 that is downloaded for 26.04
    ...
    if [ "${VERSION}" = "22.04" ] || [ "${VERSION}" = "24.04" ]; then
        ...
        apt_get_purge 120 5 300 git bison cmake flex libedit-dev libllvm18 llvm-18-dev libclang-18-dev zlib1g-dev libelf-dev libfl-dev || exit $ERR_BCC_INSTALL_TIMEOUT
    else
