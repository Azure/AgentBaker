...
func getReleaseNotes(sku, path string, fl *flags, errc chan<- error, done chan<- struct{}) {
	...
	trivyReportTableIn := filepath.Join(tmpdir, "trivy-image-table.txt")
	...
	trivyReportTableOut := filepath.Join(artifactsDirOut, fmt.Sprintf("%s-trivy-image-table.txt", fl.date))
	...
	latestTrivyReportTable := filepath.Join(artifactsDirOut, "latest-trivy-image-table.txt")
