...
testPkgDownloaded() {
  ...
  for packageVersion in "${packageVersions[@]}"; do
    ...
    if [ $OS = $UBUNTU_OS_NAME ]; then
      ...
      sysextFile=$(find "${downloadLocation}" -maxdepth 1 -name "${packageName}-${packageVersion}*-${sysextArch}.raw" -print -quit 2>/dev/null) || sysextFile=""
      if [ -z "${sysextFile}" ]; then
        err $test "System extension ${packageName}-${packageVersion} for ${sysextArch} does not exist, content of downloads dir is $(ls -al "${downloadLocation}")"
      fi
    fi
