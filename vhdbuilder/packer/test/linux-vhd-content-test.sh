...
testAMDGPUDriver() {
  ...
  if ! metadata=$(jq -ce '.AMDGPUDriver | select(type == "object")' "${COMPONENTS_FILEPATH}") ||
  ...
  if ! diagnostics_amdsmi=$(jq -er '.AMDGPUDiagnostics.amdsmiPackage | strings | select(length > 0)' "${COMPONENTS_FILEPATH}") ||
     ! diagnostics_sysdeps=$(jq -er '.AMDGPUDiagnostics.sysdepsPackage | strings | select(length > 0)' "${COMPONENTS_FILEPATH}"); then
...
testAMDGPUDiagnostics() {
  ...
  if ! metadata=$(jq -ce '.AMDGPUDiagnostics | select(type == "object")' "${COMPONENTS_FILEPATH}") ||
