...
if [[ "${UBUNTU_RELEASE}" == "22.04" ]]; then
  ...
  if [[ "${ENABLE_FIPS,,}" == "true" ]]; then
    # Purge all current kernels and dependencies, except for packages containing "fips"
    DEBIAN_FRONTEND=noninteractive apt-get remove --purge -y $(dpkg-query -W 'linux-*azure*' | awk '$2 != "" && !/fips/ { print $1 }' | paste -s)
  else
  ...
  echo "The dpkg list after purging the kernel"; dpkg -l 'linux-*azure*'
