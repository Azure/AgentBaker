...
if [ "${OS_SKU}" = "Ubuntu" ] && [ "${OS_VERSION}" = "22.04" ] && [ "$(printf %s "${ENABLE_FIPS}" | tr '[:upper:]' '[:lower:]')" = "true" ]; then
    ...
    create_fips_vm "$RESOURCE_GROUP_NAME" "$SCAN_VM_NAME" "$SCAN_VM_ADMIN_USERNAME" "$SCAN_VM_ADMIN_PASSWORD" \
        "$VHD_IMAGE" "$SCANNING_NIC_ID" "$UMSI_RESOURCE_ID" "$VM_SIZE" "$ENABLE_TRUSTED_LAUNCH" || exit $?
else
