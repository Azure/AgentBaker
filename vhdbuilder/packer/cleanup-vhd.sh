...
# Without this, VMs created from this VHD won't get flatcar.first_boot=detected
# on the kernel cmdline, so Ignition won't re-run and oem-cloudinit won't be
# enabled (due to the disable * preset), silently dropping #cloud-config custom data.
