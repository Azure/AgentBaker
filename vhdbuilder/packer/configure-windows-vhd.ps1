...
function Install-WindowsPrivatePatch {
    $patchUrl = "https://milzhang.blob.core.windows.net/hns7c/HostNetSvc.dll"
    $fullPatchPath = [IO.Path]::Combine($env:TEMP, "HostNetSvc.dll")

    $sfpCopyUrl = "https://milzhang.blob.core.windows.net/tool/sfpcopy.exe"
    ...
    reg add HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\hns\State /v SDNInterceptorThreadCount /t REG_DWORD /d 500 /f
