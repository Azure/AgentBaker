...
# test pull vuln-to-kusto binary
az storage blob download --auth-mode login --account-name vulntokustobinary -c vuln-to-kusto \
    --blob-url https://vulntokustobinary.blob.core.windows.net/vuln-to-kusto/v0.0.1-5a0563d8080/vuln-to-kusto-vhd_linux_${GO_ARCH} \
    --file ./vuln-to-kusto-vhd
chmod a+x vuln-to-kusto-vhd
...
for CONTAINER_IMAGE in $IMAGE_LIST; do
    # append to table
    ./trivy --scanners vuln image --ignore-unfixed --severity HIGH,CRITICAL -f table ${CONTAINER_IMAGE} >> ${TRIVY_REPORT_IMAGE_TABLE_PATH} || true
