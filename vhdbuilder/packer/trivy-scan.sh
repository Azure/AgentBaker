...
TRIVY_IMAGES_JSON_PATH=/opt/azure/containers/trivy-images.json
...
mkdir -p "$(dirname "${TRIVY_IMAGES_JSON_PATH}")"
...
for image in $IMAGE_LIST; do
    ...
    ./trivy --scanners vuln image --ignore-unfixed --severity HIGH,CRITICAL -f json $image >> ${TRIVY_IMAGES_JSON_PATH} || true
done
...
chmod a+r "${TRIVY_IMAGES_JSON_PATH}"
