...
if [[ $OS == $UBUNTU_OS_NAME ]]; then
  ...
  if [[ "${ENABLE_FIPS,,}" == "true" ]]; then
    ...
    dpkg --get-selections | grep -e "linux-\(headers\|modules\|image\)" | grep -v "linux-\(headers\|modules\|image\)-azure" | grep -v "$current_kernel" | tr -s '[[:space:]]' | tr '\t' ' ' | cut -d' ' -f1 | xargs -I{} apt-get remove -yq {}
  fi
