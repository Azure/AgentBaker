...
concurrency=$(azcopy env list | grep AZCOPY_CONCURRENCY_VALUE | awk '{print $2}')
echo "Concurrency value for azcopy: $concurrency"

azcopy-preview copy "${sas}" "${CLASSIC_BLOB}/${CAPTURED_SIG_VERSION}.vhd?${storage_sas_token}" --recursive=true

concurrency1=$(azcopy env list | grep AZCOPY_CONCURRENCY_VALUE | awk '{print $2}')
echo "Concurrency value for azcopy: $concurrency1"
