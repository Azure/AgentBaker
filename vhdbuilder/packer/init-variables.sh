...
# Ensure SIG and image definition names are set to their defaults if needed
...
if [[ -z "${SIG_IMAGE_NAME}" ]]; then
	SIG_IMAGE_NAME=${OS_SKU}${OS_VERSION//./}
...
# 		fi
# 		echo "No input SIG_IMAGE_NAME for Packer build output with hyperv-gen2. Setting to ${SIG_IMAGE_NAME}"
# 	fi
# fi
...
echo "checking existence of image-definition ${SIG_IMAGE_NAME} in SIG ${SIG_GALLERY_NAME} within resource group ${AZURE_RESOURCE_GROUP_NAME}..."
...
if [ -z "$id" ]; then
	...
	echo "Image definition ${SIG_IMAGE_NAME} exists in gallery ${SIG_GALLERY_NAME} within resource group ${AZURE_RESOURCE_GROUP_NAME}"
fi
...
if [ "$OS_TYPE" == "Windows" ]; then
	...
	*)
	...
	if [ -n "${WINDOWS_BASE_IMAGE_URL}" ]; then
		...
		# TODO: how can we reconcile this with removing build mode?
