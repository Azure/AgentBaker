...
REPLICATION_MODE_STRING=""
if [[ ${IMG_SKU} == "20_04-lts-cvm" ]]; then
    REPLICATION_MODE_STRING+="--replication-mode Full"
else
    REPLICATION_MODE_STRING+="--replication-mode Shallow"
fi
echo "Replication Mode: ${REPLICATION_MODE_STRING}"
...
az sig image-version create --location $AZURE_LOCATION --resource-group ${AZURE_RESOURCE_GROUP_NAME} --gallery-name PackerSigGalleryEastUS \
     ...
     --os-snapshot ${disk_snapshot_id} --tags now=${time_now} ${REPLICATION_MODE_STRING}