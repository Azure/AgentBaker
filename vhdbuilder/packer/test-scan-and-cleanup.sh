...
if [ "${LOCK_PACKER_IMAGE:-}" = "true" ]; then
  echo -e "\nCreating CanNotDelete resource lock over packer image: $SIG_VERSION"
  if ! az resource lock create -n "PackerDeleteLock" -t CanNotDelete --resource $SIG_VERSION; then
