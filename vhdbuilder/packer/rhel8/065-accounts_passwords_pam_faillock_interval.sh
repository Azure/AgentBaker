#!/usr/bin/env bash
set -e
(>&2 echo "Remediating rule 65/364: 'accounts_passwords_pam_faillock_interval'")
...
# auth required pam_faillock.so preauth is not present, insert the whole line
        ...
        control="\[success=1 default=bad\]"
        sed -i --follow-symlinks "/^auth.*$control.*pam_unix.so.*/i auth        required      pam_faillock.so preauth silent ""fail_interval"'='"$var_accounts_passwords_pam_faillock_fail_interval" "$pam_file"
fi
...
# auth default pam_faillock.so authfail is not present, insert the whole line
        ...
        control="\[success=1 default=bad\]"
        sed -i --follow-symlinks "/^auth.*$control.*pam_unix.so.*/a auth        [default=die] pam_faillock.so authfail ""fail_interval"'='"$var_accounts_passwords_pam_faillock_fail_interval" "$pam_file"
fi
