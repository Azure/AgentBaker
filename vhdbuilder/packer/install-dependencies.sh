...
if grep -q "GB200" <<< "$FEATURE_FLAGS"; then
  ...
  if [ ${UBUNTU_RELEASE} = "24.04" ] && [ ${CPU_ARCH} = "arm64" ]; then
    ...
    apt install -y nvidia-driver-575=575.57.08-0ubuntu1
    # Install DCGM exporter
    apt install -y datacenter-gpu-manager-exporter=4.1.3-1 datacenter-gpu-manager-core=1:4.3.1-1 datacenter-gpu-manager-proprietary=1:4.3.1-1 libcap2-bin
