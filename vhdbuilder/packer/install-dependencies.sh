...
installAndConfigureArtifactStreaming() {
  ...
  if isFlatcar "$OS"; then
    rpm2cpio "${MIRROR_DOWNLOAD_PATH}" | cpio -idD / opt/\* ||
      exit $ERR_ARTIFACT_STREAMING_DOWNLOAD
    rpm2cpio "${MIRROR_DOWNLOAD_PATH}" | cpio -i --to-stdout ./usr/lib/systemd/system/acr-mirror.service | install -m0644 /dev/stdin /etc/systemd/system/acr-mirror.service ||
      exit $ERR_ARTIFACT_STREAMING_DOWNLOAD
