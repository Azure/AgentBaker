...
buildNVIDIAKernelModule() {
  if [ $OS = $UBUNTU_OS_NAME ] && [ "$(isARM64)" -ne 1 ]; then # No ARM64 SKU with GPU now
    ...
    # Opt-in: pre-build the NVIDIA kernel module into the VHD. The aks-gpu container's
    # "build-only" mode compiles and DKMS-registers the kernel module + stages userspace libs
    # against THIS VHD's kernel,
    # performs NO device access (safe on the GPU-less Packer builder), and writes the marker
    # /opt/azure/aks-gpu/dkms-marker. When parking is enabled, move only its DKMS
    # registration/cache, leaving installed modules and userspace in place.
    # CSE still uses the ordinary install command.
    ...
    if grep -q "NVIDIA_CUDA_PREBAKE" <<< "$FEATURE_FLAGS"; then
      # Older CSE validation-only paths can load the installed module without restoring DKMS.
      # Keep the existing registered prebake until parking is deliberately enabled.
      local park_dkms=false
      # A repeated build may start with a previously parked tree. Never build over it and
      # later merge two registrations; restoration fails closed on an independent active tree.
      if grep -qw "NVIDIA_DKMS_PARK" <<< "$FEATURE_FLAGS"; then
        park_dkms=true
        restorePrebakedNvidiaDKMS || exit 1
      elif [ -e "${GPU_DKMS_PARKED_DIR:-/opt/azure/aks-gpu/dkms/nvidia}" ] ||
        [ -L "${GPU_DKMS_PARKED_DIR:-/opt/azure/aks-gpu/dkms/nvidia}" ] ||
        { [ -f "${GPU_DKMS_MARKER_FILE:-/opt/azure/aks-gpu/dkms-marker}" ] &&
          grep -q '^dkms_registration=' "${GPU_DKMS_MARKER_FILE:-/opt/azure/aks-gpu/dkms-marker}"; }; then
        echo "Error: parked NVIDIA prebake requires NVIDIA_DKMS_PARK and restore-capable CSE"
        exit 1
      fi
      ...
      if [ ! -f "${GPU_DKMS_MARKER_FILE:-/opt/azure/aks-gpu/dkms-marker}" ]; then
        echo "Error: NVIDIA CUDA prebake did not produce ${GPU_DKMS_MARKER_FILE:-/opt/azure/aks-gpu/dkms-marker}"
        exit 1
      fi
      # An opted-in build must not declare success with an active registration.
      if [ "${park_dkms}" = true ]; then
        parkPrebakedNvidiaDKMS || exit 1
      fi
      cat << EOF >> ${VHD_LOGS_FILEPATH}
