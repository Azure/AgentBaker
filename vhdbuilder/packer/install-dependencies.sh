...
if [[ ${UBUNTU_RELEASE} == "18.04" ]]; then
  ...
  # newer versions of moby-docker already spin up a containerd process so this step is not necessary
  if [[ ! -f "/var/run/containerd/containerd.sock" ]]; then
      containerd &>/dev/null &
      containerdPID=$!
