...
if [ "$OS" = "$UBUNTU_OS_NAME" ] && echo "$FEATURE_FLAGS" | grep -q "cvm"; then
  ...
  wait_for_apt_locks
  apt_get_install 10 2 grub-efi || exit 1
fi
...
if [[ "$IMG_SKU" != *"minimal"* ]]; then
  ...
  for apt_package in "${required_pkg_list[@]}"; do
      if ! apt_get_install 10 2 "$apt_package"; then
...
installCNI() {
    ...
    if [ "${OS}" = "${UBUNTU_OS_NAME}" ]; then
        ...
        apt_get_install 20 30 ${packageName} || exit $ERR_CNI_VERSION_INVALID
...
while IFS= read -r p; do
  ...
  case $name in
    ...
    "blobfuse"|"blobfuse2")
      for version in "${PACKAGE_VERSIONS[@]}"; do
        if isUbuntu "$OS"; then
          if ! apt_get_install 10 2 "${name}=${version}"; then
...
installAndConfigureArtifactStreaming() {
  ...
  case "$downloadURL" in
    *.deb)
      apt_get_install 10 2 "$MIRROR_DOWNLOAD_PATH" || exit $ERR_ARTIFACT_STREAMING_DOWNLOAD
      ;;
    *.rpm)
