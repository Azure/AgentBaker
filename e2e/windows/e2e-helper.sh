...
backfill_clean_storage_container() {
    ...
    versions=$(az aks get-versions --location $BUILD_AZURE_LOCATION --query "orchestrators[].orchestratorVersion" -o tsv)
    k8s_versions=${versions//./}

    # Get container names e.g. akswinstore2022-1256 (for this $container_version would be "1256")
    container_names=$(az storage container list --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY --query "[?starts_with(name, 'akswinstore')].name" -o tsv)
    ...
    for container_name in $container_names; do
        ...
        if [[ $k8s_versions == *"$container_version"* ]]; then
            echo "The version $container_version is available in the $BUILD_AZURE_LOCATION region."
        else
            echo "The version $container_version is not available in the $BUILD_AZURE_LOCATION region."
            echo "Deleting the container."
            az storage container delete --name $container_name --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY
...
create_storage_container() {
    ...
    exists=$(az storage container exists --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY --name $WINDOWS_E2E_STORAGE_CONTAINER)
    if [[ $exists == *false* ]]; then
        az storage container create -n $WINDOWS_E2E_STORAGE_CONTAINER --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY
        echo "Created storage container $WINDOWS_E2E_STORAGE_CONTAINER in $E2E_STORAGE_ACCOUNT_NAME"
    else
        # check if the storage container is empty and delete the blobs within one by one if not
        blob_list=$(az storage blob list --account-name $E2E_STORAGE_ACCOUNT_NAME --container-name $WINDOWS_E2E_STORAGE_CONTAINER --account-key $MAPPED_ACCOUNT_KEY -o json | jq -r '.[] | .name')
        if [[ -n $blob_list ]]; then
            for blob in $blob_list; do
                az storage blob delete --account-name $E2E_STORAGE_ACCOUNT_NAME --container-name $WINDOWS_E2E_STORAGE_CONTAINER --account-key $MAPPED_ACCOUNT_KEY --name $blob
                echo "Deleted blob $blob from storage container $WINDOWS_E2E_STORAGE_CONTAINER in the storage account $E2E_STORAGE_ACCOUNT_NAME"
            done
        fi
    fi
    set -x
...
upload_linux_file_to_storage_account() {
    ...
    token=$(az storage container generate-sas --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY --permissions 'w' --expiry $expiryTime --name $WINDOWS_E2E_STORAGE_CONTAINER)
...
download_linux_file_from_storage_account() {
    ...
    token=$(az storage container generate-sas --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY --permissions 'rl' --expiry $expiryTime --name $WINDOWS_E2E_STORAGE_CONTAINER)
    tokenWithoutQuote=${token//\"}
    linuxFileURL="https://${E2E_STORAGE_ACCOUNT_NAME}.blob.core.windows.net/${WINDOWS_E2E_STORAGE_CONTAINER}/${MC_VMSS_NAME}-linux-file.zip?${tokenWithoutQuote}"
...
cleanupOutdatedFiles() {
    ...
    do 
        result=$(az storage blob list -c $CONTAINER_NAME --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY -o json \
        | jq -r --arg time "$dateOfdeadline" '.[] | select(.properties.creationTime < $time)' \
        | jq -r '.name')

        for item in $result
        do
            az storage blob delete -c $CONTAINER_NAME --account-name $E2E_STORAGE_ACCOUNT_NAME --account-key $MAPPED_ACCOUNT_KEY -n $item
