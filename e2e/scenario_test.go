...
func Test_AzureLinuxV3_PodSandboxing(t *testing.T) {
	RunScenario(t, &Scenario{
		Description: "tests that a new azure linux v3 node with pod sandboxing (Kata) can be properly bootstrapped without artifact streaming",
		Config: Config{
			Cluster: ClusterKubenet,
...
func Test_AzureLinuxV3_PodSandboxing_Scriptless(t *testing.T) {
	RunScenario(t, &Scenario{
		Description: "tests that a new azure linux v3 node with pod sandboxing (Kata) can be properly bootstrapped without artifact streaming",
		Tags: Tags{
			Scriptless: true,
