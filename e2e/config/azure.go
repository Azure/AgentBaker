...
// getTagValueCI performs a case-insensitive lookup of a tag value from a gallery image version's tags.
func getTagValueCI(tags map[string]*string, key string) (string, bool) {
	for k, v := range tags {
...
// PreviouslyBuiltSIGVersion resolves the most recent prior main-branch build
// in the build gallery, excluding the current pipeline run. If
// Config.PreviouslyBuiltVHDBuildID is set, it pins to that specific build instead.
func (a *AzureClient) PreviouslyBuiltSIGVersion(ctx context.Context, image *Image, location string) (VHDResourceID, error) {
	toolkit.Logf(ctx, "Looking up previously built SIG version in %s", image.azurePortalImageUrl())

	imageVersionsClient, err := armcompute.NewGalleryImageVersionsClient(image.Gallery.SubscriptionID, a.Credential, a.ArmOptions)
	if err != nil {
		return "", fmt.Errorf("failed to create image versions client: %w", err)
	}
	...
	overrideBuildID := Config.PreviouslyBuiltVHDBuildID
	...
	for pager.More() {
		...
		for _, version := range page.Value {
			if version.Properties == nil || version.Properties.ProvisioningState == nil {
				continue
			}
			state := *version.Properties.ProvisioningState
			if state != armcompute.GalleryProvisioningStateSucceeded && state != armcompute.GalleryProvisioningStateUpdating {
				continue
			}

			// skip versions tagged for no-selection
			if _, ok := version.Tags[noSelectionTagName]; ok {
				toolkit.Logf(ctx, "Skipping version %s: has %s tag", *version.ID, noSelectionTagName)
				continue
			}

			// if override is set, find the exact match
			if overrideBuildID != "" {
				bid, ok := getTagValueCI(version.Tags, "buildId")
				if ok && bid == overrideBuildID {
					toolkit.Logf(ctx, "Found pinned version by PreviouslyBuiltVHDBuildID=%s: %s", overrideBuildID, *version.ID)
					image.Version = *version.Name
					if err := a.ensureReplication(ctx, image, version, location); err != nil {
						return "", fmt.Errorf("failed ensuring image replication: %w", err)
					}
			...
			// filter to main-branch builds only
			branch, ok := getTagValueCI(version.Tags, "branch")
			if !ok || branch != "refs/heads/main" {
				continue
			}

			// exclude current build
			bid, _ := getTagValueCI(version.Tags, "buildId")
			if bid == currentBuildID {
				toolkit.Logf(ctx, "Skipping version %s: matches current build ID %s", *version.ID, currentBuildID)
				continue
			}

			// parse "now" tag (unix epoch) for sorting
			nowStr, ok := getTagValueCI(version.Tags, "now")
			if !ok {
				continue
			}
			nowVal, err := strconv.ParseInt(nowStr, 10, 64)
			if err != nil {
				toolkit.Logf(ctx, "Skipping version %s: malformed 'now' tag %q", *version.ID, nowStr)
				continue
			}

			candidates = append(candidates, candidate{version: version, nowTag: nowVal})
		}
	}

	if overrideBuildID != "" {
		return "", fmt.Errorf("could not find version with buildId=%s in %s: %w", overrideBuildID, image.azurePortalImageUrl(), ErrNotFound)
	}

	if len(candidates) == 0 {
		return "", fmt.Errorf("no previously built main-branch versions found in %s: %w", image.azurePortalImageUrl(), ErrNotFound)
	}

	// sort descending by now tag (most recent first)
	slices.SortFunc(candidates, func(a, b candidate) int {
		return cmp.Compare(b.nowTag, a.nowTag)
	})

	selected := candidates[0]
	toolkit.Logf(ctx, "Selected previously built version: %s (now=%d, buildId=%s)",
		*selected.version.ID,
		selected.nowTag,
		func() string { v, _ := getTagValueCI(selected.version.Tags, "buildId"); return v }(),
	)

	image.Version = *selected.version.Name
	if err := a.ensureReplication(ctx, image, selected.version, location); err != nil {
		return "", fmt.Errorf("failed ensuring image replication: %w", err)
	}

	return VHDResourceID(*selected.version.ID), nil
}
