...
// COSIs can be multi-GB, and the OS temp dir (t.TempDir(), which resolves
// under os.TempDir()/"/tmp") is too small on some ADO runners. When the ADO
// agent's own temp directory is available (AGENT_TEMPDIRECTORY, set via
...
func cosiDownloadDir(t *testing.T) string {
	t.Helper()

	if dir := os.Getenv("AGENT_TEMPDIRECTORY"); dir != "" {
		downloadDir, err := os.MkdirTemp(dir, "acl-cosi-*")
		require.NoError(t, err, "creating COSI download directory under AGENT_TEMPDIRECTORY")
...
func ValidateACLCOSI(t *testing.T, cosiURL string) {
	...
	localPath := filepath.Join(cosiDownloadDir(t), "acl.cosi")
