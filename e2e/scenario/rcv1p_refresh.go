...
func runRefreshHealthStages(ctx context.Context, stages []refreshHealthStage) error {
	for _, stage := range stages {
		...
		logging.Logf(ctx, "RCV1P refresh health: %s", stage.name)
...
func ValidateRCV1PRefreshHealth(ctx context.Context, s *Scenario) error {
	...
	refresh := func(ctx context.Context) error {
		...
		logging.Logf(ctx, "Real refresh result=%s trust SHA256 before=%s after=%s containerd before=%+v after=%+v; unchanged roots prove idempotence, not forced platform rotation",
...
func validateRCV1PRefreshHash(ctx context.Context, output string, want *rcv1pRefreshArtifact) error {
	...
	logging.Logf(ctx, "Installed RCV1P artifact path=%s SHA256=%s; expected %s SHA256=%s",
...
func runInstalledRCV1PRefresh(ctx context.Context, s *Scenario) (string, error) {
	...
	logging.Logf(ctx, "Invoking installed RCV1P refresh: %s (verified script %s)", command, installedRCV1PScript)
	...
	if err == nil {
		logging.Logf(ctx, "Real installed refresh completed: RCV1P mode, opted in, acquired certificates, installed matching anchors; no forced rotation")
	}
	if err != nil {
		return "", err
	}
