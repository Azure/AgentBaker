...
func Test_Ubuntu2404_DraDriverNvidiaGpuRunning_AKSNodeController(t *testing.T) {
	RunScenario(t, &Scenario{
		...
		Config: Config{
			Cluster: ClusterKubenet,
			VHD:     config.VHDUbuntu2404Gen2Containerd,
			// TEST-ONLY (xuxue/noskip-prvhd-test): SkipScriptlessEnvParity removed so the parity
			// check runs. Run against a PR-built VHD (VHD_BUILD_ID) so the extension's VHD-baked ANC
			// is the PR's ANC and the check compares PR-ANC vs PR-baker.
			BootstrapConfigMutator: func(_ *Cluster, nbc *datamodel.NodeBootstrappingConfiguration) {
				nbc.AgentPoolProfile.VMSize = "Standard_NV6ads_A10_v5"
				nbc.ConfigGPUDriverIfNeeded = true
				nbc.EnableNvidia = true
