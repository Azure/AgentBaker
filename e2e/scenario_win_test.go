...
func Test_NetworkIsolatedCluster_Windows_OrasContainerdDownload(t *testing.T) {
	RunScenario(t, &Scenario{
		Description: "Tests that Windows nodes in network isolated clusters download containerd binaries via ORAS when BootstrapProfileContainerRegistryServer is set",
		Tags: Tags{
			NetworkIsolated: true,
			NonAnonymousACR: false,
		},
		Config: Config{
			...
			Validator: func(ctx context.Context, s *Scenario) {
				ValidateFileHasContent(ctx, s, "/k/kubeletstart.ps1", "--container-runtime=remote")
				// Verify containerd binaries were downloaded via ORAS instead of HTTP
