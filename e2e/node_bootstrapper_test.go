...
func CSENodeBootstrapper(ctx context.Context, t *testing.T, cluster *Cluster) string {
	...
	require.NoError(t, err)
	return fmt.Sprintf(`(echo '%s' | base64 -d > config.json && curl -L -o ./node-bootstrapper '%s' && chmod +x ./node-bootstrapper && mkdir -p /var/log/azure && ./node-bootstrapper provision --provision-config=config.json) > /var/log/azure/node-bootstrapper.log 2>&1`, base64.StdEncoding.EncodeToString(configJSON), url)
}
