...
func ValidateKubeletHasCLIFlag(ctx context.Context, s *Scenario, flagName, flagValue string) {
  configFileFlags := fmt.Sprintf("FLAG: --%s=\"", flagName)
  grepCmdLine := fmt.Sprintf("sudo journalctl -u kubelet | grep '%s'", configFileFlags)
  execResult := execScriptOnVMForScenarioValidateExitCode(ctx, s, grepCmdLine, 0, "could not retrieve kubelet logs with journalctl")
  actualFlagValue := execResult.stdout.String()
  //and assert the line contains the val
  require.Containsf(s.T, actualFlagValue, flagValue, "expected to find flag %s contains value %s, instead %s.",
    flagName, flagValue, actualFlagValue)
}
