...
func ValidateScriptlessNBCCSECmd(ctx context.Context, s *Scenario) {
	nbc := s.Runtime.NBC
	if nbc != nil && nbc.EnableScriptlessNBCCSECmd && !nbc.PreProvisionOnly && s.VHD.SupportsScriptless() && (nbc.CustomCATrustConfig == nil || len(nbc.CustomCATrustConfig.CustomCATrustCerts) == 0) {
