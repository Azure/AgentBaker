	...
	if s.Runtime.AKSNodeConfig != nil && usesScriptlessNBCCSECmd(s) {
		...
		if !fileHasContent(ctx, s, logFile, "env compare: no differences found between provision-config and nbc-cmd env vars") {
			...
			if len(diffVars) == 0 {
				...
				s.T.Fatalf("expected env compare to report no differences between provision-config and nbc-cmd, "+
			...
			if unexpected := unexpectedEnvCompareDiffVars(diffVars); len(unexpected) > 0 {
				s.T.Fatalf("expected no env var differences between provision-config and nbc-cmd, but found differences in %v:\n%s",
