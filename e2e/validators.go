...
func ValidateKernelLogs(ctx context.Context, s *Scenario) {
	...
	patterns := map[string]categoryPattern{
		...
		"IO/FS": {
			...
			exclude: `sr[0-9]|Shutdown timeout set to|duplicate device`,
		},
	}
...
//  4. Full preset file listing across all preset directories
...
systemctl status systemd-preset-all.service 2>&1 | head -15 || echo "systemd-preset-all.service not found"
systemctl cat systemd-preset-all.service 2>&1 || echo "Cannot cat systemd-preset-all.service"

echo ""
echo "===== 5. MACHINE-ID (ConditionFirstBoot) ====="
...
if [ -f "$TAR" ]; then
    echo "EXISTS: $TAR (size: $(stat -c%s "$TAR") bytes)"
    echo "Contents:"
    tar tf "$TAR" 2>/dev/null | head -20 || echo "  failed to list tar contents"
else
    echo "MISSING: $TAR (already extracted or never delivered)"
fi
echo "--- /var/lib/ignition/ listing ---"
...
echo ""
echo "===== 12. PRESET DRY-RUN ====="
echo "What would systemctl preset do for our services:"
...
`
	...
	s.T.Logf("Issue 9 diagnostics exit code: %s\n%s", result.exitCode, result.stdout)
	if result.stderr != "" {
		s.T.Logf("Issue 9 diagnostics stderr:\n%s", result.stderr)
	}
