...
func ValidateNPDGPUCountCondition(ctx context.Context, s *Scenario) {
	...
	require.NoError(s.T, err, "timed out waiting for GpuCountGood condition to appear on node %q", s.Runtime.KubeNodeName)
...
func ValidateNPDGPUCountAfterFailure(ctx context.Context, s *Scenario) {
	...
	require.NoError(s.T, err, "timed out waiting for GPUCountBad condition to appear on node %q", s.Runtime.KubeNodeName)
