...
payload file is removed (not committed) — baker treats a missing file as "no active script
hotfix" and ships nothing, so download-hotfix is a no-op.
...
PAYLOAD_HEADER = (
    "#cloud-config\n"
    "# Script hotfix write_files payload. Auto-generated by hotfix/hotfix_generate.py.\n"
...
def reset_payload():
    """Remove the payload file so there is no active script hotfix.

    baker treats a missing file as "ship nothing", so the payload is never committed on
    normal builds; it only exists on official/* branches while a script hotfix is active.
    """
    if os.path.exists(OUTPUT):
        os.remove(OUTPUT)
        print(f"Removed {OUTPUT} (no active script hotfix).", file=sys.stderr)
    else:
        print(f"{OUTPUT} already absent (no active script hotfix).", file=sys.stderr)
