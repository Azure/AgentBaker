...
kubelet_update() {
    ...
    if [ "${custom_patching}" = "true" ]; then
        ...
        imds_token=$(curl -s -H Metadata:true --noproxy "*" "http://169.254.169.254/metadata/attested/document?api-version=2025-04-07" | jq -r .signature)
        if [ -z "${imds_token}" ] || [ "${imds_token}" = "null" ]; then
            echo "cannot get IMDS token"
            return 1
        fi
        endpoint="aks-security-patch.data.mcr.microsoft.com"
        response=$(curl -s -w "%{http_code}" -H "Authorization: ${imds_token}" --cacert ${CLUSTER_CA_CERT} --resolve ${endpoint}:443:${cluster_ip} https://${endpoint}/v1/packages)
        packages="${response::-3}"
        status="${response: -3}"
        if [ "${status}" != "200" ]; then
    ...
    if [ "${custom_patching}" = "true" ]; then
        ...
        curl_output=$(curl -fsSL -o "${SECURITY_PATCH_TMP_DIR}/${package_name}" "${kubelet_url_with_token}" 2>&1)
        curl_exit_code=$?
