...
tdnf_download() {
    ...
    for i in $(seq 1 $retries); do
        ! (tdnf install "$package_name" -y --downloadonly --downloaddir "$download_dir" 2>&1 | tee $dnf_download_output | grep -E "^([WE]:.*)|([eE]rr.*)$") && \
...
kubelet_update() {
    ...
    tdnf_download kubelet-${target_kubelet_version} ${SECURITY_PATCH_TMP_DIR}
