...
pullContainerImage() {
    ...
    if [ "${CLI_TOOL,,}" = "ctr" ]; then
        retrycmd_if_failure $PULL_RETRIES $PULL_WAIT_SLEEP_SECONDS $PULL_TIMEOUT_SECONDS ctr --namespace k8s.io image pull $CONTAINER_IMAGE_URL --platform linux/${CPU_ARCH}
        code=$?
