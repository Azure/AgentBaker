...
installContainerdWasmShims(){
    ...
    for version in "${package_versions[@]}"; do
        ...
        logs_to_events "AKS.CSE.logDownloadURL" containerd_wasm_url=$(update_base_url $containerd_wasm_urlL)
...
installCNI() {
    ...
    if [ ! -f "$COMPONENTS_FILEPATH" ] || ! jq '.Packages[] | select(.name == "cni-plugins")' < $COMPONENTS_FILEPATH > /dev/null; then
        ...
        retrycmd_get_tarball 120 5 "${CNI_DOWNLOADS_DIR}/refcni.tar.gz" "https://${PACKAGE_DOWNLOAD_BASE_URL}/cni-plugins/v1.4.1/binaries/cni-plugins-linux-amd64-v1.4.1.tgz"
