...
# installWALinuxAgent installs the WALinuxAgent version specified in components.json from GitHub
# This allows us to run the latest waagent version regardless of what's in the OS package repos
# Works on Ubuntu, Mariner, and Azure Linux
installWALinuxAgent() {
    local WAAGENT_DOWNLOADS_DIR="/opt/walinuxagent/downloads"

    walinuxagentPackage=$(jq ".Packages" "$COMPONENTS_FILEPATH" | jq ".[] | select(.name == \"walinuxagent\")") || {
        echo "WARNING: walinuxagent package not found in components.json, skipping installation"
        return 0
    }

    if [ -z "$walinuxagentPackage" ] || [ "$walinuxagentPackage" = "null" ]; then
        echo "WARNING: walinuxagent package not defined in components.json, skipping installation"
        return 0
    fi

    # Get the version from the default downloadURIs
    local TARGET_VERSION
    TARGET_VERSION=$(echo "$walinuxagentPackage" | jq -r '.downloadURIs.default.current.versionsV2[0].latestVersion // empty')

    if [ -z "$TARGET_VERSION" ] || [ "$TARGET_VERSION" = "null" ]; then
        echo "WARNING: walinuxagent version not found in components.json, skipping installation"
        return 0
    fi

    echo "Installing WALinuxAgent version ${TARGET_VERSION} from GitHub..."

    # Check current version
    local CURRENT_VERSION=""
    if command -v waagent &> /dev/null; then
        CURRENT_VERSION=$(waagent --version 2>/dev/null | grep -oP 'WALinuxAgent-\K[0-9.]+' | head -n1 || true)
    fi

    if [ "${CURRENT_VERSION}" = "${TARGET_VERSION}" ]; then
        echo "WALinuxAgent ${TARGET_VERSION} is already installed, skipping"
        return 0
    fi
    ...
    tar -xzf "${TARBALL}" -C "${WAAGENT_DOWNLOADS_DIR}" || {
    ...
    echo "Successfully installed WALinuxAgent ${TARGET_VERSION}"
    return 0
}
