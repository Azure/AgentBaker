...
/usr/bin/nohup /bin/bash -c "/bin/bash /opt/azure/containers/provision.sh >> /var/log/azure/cluster-provision.log 2>&1 || exit $?; systemctl --no-pager -l status kubelet 2>&1 | head -n 100"