...
apt_get_dist_upgrade() {
  ...
  for i in $(seq 1 $retries); do
    ...
    apt-get -f -y install
    apt-mark showhold
    ! (apt-get -o Acquire::Retries=10 -o APT::Get::Always-Include-Phased-Updates=true -o Acquire::http::Timeout=60 -o Debug::pkgProblemResolver=true -o Dpkg::Options::="--force-confnew" dist-upgrade -y 2>&1 | tee $apt_dist_upgrade_output | grep -E "^([WE]:.*)|([Ee][Rr][Rr][Oo][Rr].*)$") && \
