...
installDeps() {
    ...
    pkg_list=(ca-certificates ceph-common cifs-utils conntrack cracklib-runtime ebtables ethtool git glusterfs-client htop init-system-helpers inotify-tools iotop iproute2 ipset iptables nftables jq libpam-pwquality mount nfs-common socat sysfsutils sysstat util-linux xz-utils netcat-openbsd zip kmod gcc make dkms initramfs-tools linux-headers-$(uname -r) linux-modules-extra-$(uname -r))
