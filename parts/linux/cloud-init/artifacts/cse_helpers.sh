...
ERR_MS_PROD_DEB_DOWNLOAD_TIMEOUT=42 # Timeout waiting for https://packages.microsoft.com/config/ubuntu/${UBUNTU_RELEASE}/packages-microsoft-prod.deb
...
# === TEST-ONLY (ganesh/prebake-pipeline-test): point the node-boot driver images at the
# anonymous-pull test ACR builds that carry the unmerged aks-gpu PR #162 changes. CUDA at the same
# 580.126.09 version baked into the VHD so the marker matches; GRID at 570.211.01 so that GRID nodes
# (routed to a full install by the driver_kind guard) exercise the lib-clear fix that prevents the
# prebaked CUDA libs from mixing with the GRID libs. DO NOT MERGE. ===
if [ "${NVIDIA_GPU_DRIVER_TYPE}" = "cuda" ]; then
    export NVIDIA_DRIVER_IMAGE="gpuprebaketest47729.azurecr.io/aks-gpu-cuda"
    export NVIDIA_DRIVER_IMAGE_TAG="580.126.09-test6"
elif [ "${NVIDIA_GPU_DRIVER_TYPE}" = "grid" ]; then
    export NVIDIA_DRIVER_IMAGE="gpuprebaketest47729.azurecr.io/aks-gpu-grid"
    export NVIDIA_DRIVER_IMAGE_TAG="570.211.01-test1"
fi
