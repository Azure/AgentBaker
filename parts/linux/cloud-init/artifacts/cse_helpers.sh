...
fetch_and_cache_imds_instance_metadata() {
    ...
    local body
    body=$(curl -fsSL -H "Metadata: true" --noproxy "*" --retry 20 --retry-delay 2 --retry-connrefused "http://169.254.169.254/metadata/instance?api-version=2021-02-01")
    if [ "$?" -ne 0 ]; then
...
get_imds_network_metadata() {
    ...
    jq -r '.network.interface' "$IMDS_INSTANCE_METADATA_CACHE_FILE"
}
...
get_primary_nic_ip() {
    ...
    jq -r '.network.interface[0].ipv4.ipAddress[0].privateIpAddress // ""' "$IMDS_INSTANCE_METADATA_CACHE_FILE"
}
