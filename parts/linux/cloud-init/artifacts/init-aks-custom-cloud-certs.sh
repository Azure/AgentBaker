...
# Certificate bootstrap and refresh for AKS custom clouds.
...
# Returns 0 when opted in, 1 when not opted in, and 2 when wireserver is unreachable.
...
function detect_certificate_os {
...
function refresh_custom_cloud_certs {
    local refresh_location="$1"
    local location_normalized="${refresh_location,,}"
...
function install_ca_refresh_schedule {
    local script_path="$1"
    local location="$2"

    if [ "$IS_UBUNTU" -eq 1 ] || [ "$IS_MARINER" -eq 1 ] || [ "$IS_AZURELINUX" -eq 1 ]; then
        local new_entry="0 19 * * * \"$script_path\" ca-refresh \"$location\""
        local existing
        local filtered
        local old_script_path
        old_script_path="$(dirname -- "$script_path")/init-aks-cloud.sh"
        existing=$(crontab -l 2>/dev/null || true)
        filtered=$(printf '%s\n' "$existing" | grep -F -v "\"$old_script_path\" ca-refresh" | grep -F -v "\"$script_path\" ca-refresh" || true)
        ...
        local service_path="/etc/systemd/system/azure-ca-refresh.service"
        local timer_path="/etc/systemd/system/azure-ca-refresh.timer"
...
function init_aks_custom_cloud_certs_main {
    local action="${1:-init}"
    local refresh_location="${2:-${LOCATION:-}}"
    local script_path

    detect_certificate_os || return 1
    refresh_custom_cloud_certs "$refresh_location" || return 1
    ...
    if [ "$INSTALL_CA_REFRESH_SCHEDULE" -eq 1 ]; then
        ...
        install_ca_refresh_schedule "$script_path" "${LOCATION:-}"
    fi
    return 0
}
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    set -x
    init_aks_custom_cloud_certs_main "$@"
fi
