...
# This is a wrapper script for the check_dns_issues.sh script
# It executes the check_dns_issues.sh script with a shorter timeout than NPD
# and manages exit codes appropriately.

SCRIPT_DIR="$(dirname "${BASH_SOURCE[0]}")"
MAIN_SCRIPT_PATH="${SCRIPT_DIR}/check_dns_issues.sh"
JSON_CONFIG_PATH="${SCRIPT_DIR}/../custom-plugin-monitor/custom-dns-issue-monitor.json"

# Default timeout value in case of any issues reading monitor
DEFAULT_TIMEOUT_SECONDS=55

if [ ! -f "$MAIN_SCRIPT_PATH" ]; then
    echo "ERROR: Check script not found: $MAIN_SCRIPT_PATH" >&2
...
echo "Running check_dns_issues.sh with timeout of ${SCRIPT_TIMEOUT_SECONDS}s (NPD timeout: ${NPD_TIMEOUT_SECONDS}s)"
...
if [ $RETURN_CODE -eq 124 ]; then
    echo "check_dns_issues.sh timed out (return code 124) after ${SCRIPT_TIMEOUT_SECONDS}s. Still exiting with 0." >&2
    exit 0
fi

echo "check_dns_issues.sh completed with return code $RETURN_CODE" >&2
