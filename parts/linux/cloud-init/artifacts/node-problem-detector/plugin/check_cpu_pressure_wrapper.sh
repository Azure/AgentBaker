...
# This is a wrapper script for the check_cpu_pressure.sh script
# It executes the check_cpu_pressure.sh script with a shorter timeout than NPD
# and ensures it always exits with status code 0 to prevent unwanted events

SCRIPT_DIR="$(dirname "${BASH_SOURCE[0]}")"
FULL_SCRIPT_PATH="${SCRIPT_DIR}/check_cpu_pressure.sh"
...
echo "Running check_cpu_pressure.sh via wrapper"
...
if [ $RETURN_CODE -eq 124 ]; then
    echo "check_cpu_pressure.sh timed out (return code 124)" >&2
elif [ $RETURN_CODE -ne 0 ]; then
    echo "check_cpu_pressure.sh failed with return code $RETURN_CODE" >&2
fi
