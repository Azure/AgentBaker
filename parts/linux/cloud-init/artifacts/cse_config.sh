...
getKubeletActiveFlagsJSON() {
    ...
    # If kubelet loaded a config file, include its full content. The file is already a compact
    # subset generated by AgentBaker (only RP-specified overrides), not the full kubelet default
    # config, so it fits within the Context1 size limit without further filtering.
    local config_file_json="{}"
    if [ "${uses_config_file}" = "true" ] && [ -n "${config_path}" ] && [ -f "${config_path}" ]; then
        config_file_json="$(jq -c '.' "${config_path}" 2>/dev/null || echo '{}')"
    fi
    ...
    if [ "${payload_bytes}" -gt "${max_message_bytes}" ]; then
        echo "kubelet active flags payload exceeded ${max_message_bytes} bytes (${payload_bytes}), dropping config_file content" >&2
        ...
        payload="$(jq -cn \
            ...
            --argjson flag_count "${flag_count}" \
            --argjson flags "${flags_json}" \
            '{found: $found, uses_config_file: $uses_config_file, config_path: $config_path, flag_count: $flag_count, flags: $flags, config_file: "truncated:exceeded_size_cap"}')"
    fi
    echo "${payload}"
}
