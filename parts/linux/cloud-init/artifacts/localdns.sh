...
replace_azurednsip_in_corefile() {
    local UPSTREAM_VNET_DNS_SERVERS="${1:-}"
    if [ -z "${UPSTREAM_VNET_DNS_SERVERS}" ] || [ "$UPSTREAM_VNET_DNS_SERVERS" = '""' ]; then
        echo "No upstream VNET DNS servers supplied."
        return 1
    fi
...
wait_for_localdns_removed_from_resolv_conf() {
    ...
    local iteration=0 current_dns=""

    echo "Waiting for localdns (${LOCALDNS_NODE_LISTENER_IP}) to be removed from resolv.conf..." >&2

    while [ "$iteration" -lt "$max_iterations" ]; do
        # Use word boundary matching (-w) with fixed string (-F) to avoid partial IP matches.
        if current_dns=$(awk '/^nameserver/ {print $2}' "$RESOLV_CONF" 2>/dev/null | paste -sd' ') &&
            [ -n "$current_dns" ] && ! grep -qwF "$LOCALDNS_NODE_LISTENER_IP" <<< "$current_dns"; then
            echo "DNS configuration refreshed successfully. Current DNS: ${current_dns}" >&2
            printf '%s\n' "$current_dns"
            return 0
        fi
    ...
    echo "Timed out waiting for localdns to be removed from resolv.conf after ${max_wait_seconds} seconds." >&2
    echo "Current DNS: ${current_dns}" >&2
    return 1
}
...
if ! UPSTREAM_VNET_DNS_SERVERS=$(wait_for_localdns_removed_from_resolv_conf 5); then
...
# ---------------------------------------------------------------------------------------------------------------------
replace_azurednsip_in_corefile "$UPSTREAM_VNET_DNS_SERVERS" || exit $ERR_LOCALDNS_FAIL
