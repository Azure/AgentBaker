...
# Report whether the CoreDNS binary understands the cache plugin's serve_stale_policy
# directive, which landed upstream in CoreDNS 1.14.7 (coredns/coredns#8378).
# "coredns --version" prints a line such as "CoreDNS-1.14.7". Anything that cannot be
# parsed is treated as unsupported: CoreDNS aborts at startup on an unknown directive, so
# the safe answer whenever the version is unknown is "no".
# Returns 0 when supported, 1 otherwise.
coredns_supports_serve_stale_policy() {
    local version_output version major minor patch
    version_output="$("${COREDNS_BINARY_PATH}" --version 2>/dev/null | head -n 1)"
    version="$(echo "${version_output}" | sed -n 's/.*CoreDNS-\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\).*/\1/p')"
    if [ -z "${version}" ]; then
        echo "Could not determine CoreDNS version from '${version_output}'."
        return 1
    fi
    ...
    if [ "${major}" -eq 1 ] && [ "${minor}" -gt 14 ]; then
        return 0
    fi
    if [ "${major}" -eq 1 ] && [ "${minor}" -eq 14 ] && [ "${patch}" -ge 7 ]; then
        return 0
    fi
    return 1
}

# Drop serve_stale_policy from the corefile when the CoreDNS on this node is too old to
# parse it. The corefile is generated by the RP, which cannot see which CoreDNS version
# the VHD baked in, so this is the only place the two can be reconciled. Leaving an
# unsupported directive in place would stop CoreDNS from starting at all, which takes DNS
# down on the node -- a far worse outcome than silently losing the policy.
remove_unsupported_serve_stale_policy() {
    local corefile="$1"

    if ! grep -qE '^[[:space:]]*serve_stale_policy([[:space:]]|$)' "${corefile}"; then
        return 0
    fi

    if coredns_supports_serve_stale_policy; then
        return 0
    fi

    echo "CoreDNS at ${COREDNS_BINARY_PATH} does not support serve_stale_policy, removing it from ${corefile}."
    sed -i -e '/^[[:space:]]*serve_stale_policy\([[:space:]]\|$\)/d' "${corefile}" || {
        echo "Failed to remove serve_stale_policy from ${corefile}."
        return 1
    }
    return 0
}
...
replace_azurednsip_in_corefile() {
    ...
    remove_unsupported_serve_stale_policy "${UPDATED_LOCALDNS_CORE_FILE}" || return 1
...
#
# The reload is treated as converged once resolv.conf shows the state the drop-in asks for AND
# the upstream servers are routable again:
#   - resolv.conf is non-empty, lists the localdns listener, and no longer lists any upstream
#     server. 'DNS=<listener>' plus 'UseDNS=false' makes that networkd's deterministic end
#     state, so this is the applied-the-drop-in signal, not a heuristic.
#   - every upstream server has a route, so localdns has egress to forward to.
#
# The non-empty requirement carries its weight: during a reload resolv.conf can transiently
# contain no nameservers at all. Returning then would signal ready with no resolver configured
...
wait_for_network_reload_settled() {
    ...
    while [ "$iteration" -lt "$max_iterations" ]; do
        ...
        if [ -n "$current_dns" ] &&
           ...
           ! upstream_dns_servers_listed "$upstream_dns_servers" "$current_dns" &&
           upstream_dns_servers_routable "$upstream_dns_servers"; then
