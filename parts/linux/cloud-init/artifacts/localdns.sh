...
# Build iptables rules to skip conntrack for DNS traffic to and from localdns.
build_localdns_iptable_rules() {
    # These rules skip conntrack for DNS requests to, and responses from, the local DNS service IPs to save conntrack table space.
    # OUTPUT destination rules affect node services and hostNetwork: true pods.
    # PREROUTING destination rules affect traffic from regular pods.
    ...
    # Localdns responses are generated by the local coredns process, so response packets traverse OUTPUT.
    # They do not traverse PREROUTING, which only handles packets arriving from the network before local routing.
    for IP in ${LOCALDNS_NODE_LISTENER_IP} ${LOCALDNS_CLUSTER_LISTENER_IP}; do
        for PROTO in tcp udp; do
            IPTABLES_RULES+=("OUTPUT -p ${PROTO} -s ${IP} --sport 53 -j NOTRACK")
        done
    done
}
