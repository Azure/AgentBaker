...
if [ -f "${AKSLOCALDNS_ENV_FILE_PATH}" ]; then
    ...
    printf "Error: akslocaldns envfile does not exist at ${AKSLOCALDNS_ENV_FILE_PATH}.\n"
    exit 1
fi
...
if [ ! -f "${AKSLOCALDNS_CORE_FILE_PATH}" ]; then
  printf "Error: akslocaldns corefile does not exist at ${AKSLOCALDNS_CORE_FILE_PATH}.\n"
  exit 1
fi
if [ ! -s "${AKSLOCALDNS_CORE_FILE_PATH}" ]; then
  printf "Error: akslocaldns corefile is empty at ${AKSLOCALDNS_CORE_FILE_PATH}.\n"
  exit 1
fi

# Validate UPSTREAM_VNET_DNS_SERVERS is not empty
if [ -z "${UPSTREAM_VNET_DNS_SERVERS}" ]; then
    echo "Error: No Upstream VNET DNS servers found in /run/systemd/resolve/resolv.conf.\n"
    exit 1
fi
...
if [ ! -f "$AKSLOCALDNS_SLICE_PATH" ]; then
    printf "Error: akslocaldns slice file does not exist at ${AKSLOCALDNS_SLICE_PATH}.\n"
    exit 1
fi
    ...
    if [ ! -z "${COREDNS_PID:-}" ]; then
        if ps ${COREDNS_PID} >/dev/null; then
            if [[ ${AKSLOCALDNS_SHUTDOWN_DELAY} -gt 0 ]]; then
                ...
                printf "sleeping ${AKSLOCALDNS_SHUTDOWN_DELAY} seconds to allow connections to terminate.\n"
...
if [ ! -x "${SCRIPT_PATH}/coredns" ]; then
    printf "extracting coredns from image: ${AKSLOCALDNS_IMAGE_URL}.\n"
    CTR_TEMP="$(mktemp -d)"
    ...
    if ! ctr -n k8s.io images ls | grep -q "${AKSLOCALDNS_IMAGE_URL}"; then
        printf "Image not found locally, attempting to pull: ${AKSLOCALDNS_IMAGE_URL}.\n"
...
printf "setting up akslocaldns dummy interface with IPs ${AKSLOCALDNS_NODE_LISTENER_IP} and ${AKSLOCALDNS_CLUSTER_LISTENER_IP}.\n"
...
printf "starting akslocaldns: ${COREDNS_COMMAND}.\n"
...
printf "akslocaldns PID is ${COREDNS_PID}\n"
...
# --------------------------------------------------------------------------------------------------------------------
printf "updating network DNS configuration to point to akslocaldns via ${NETWORK_DROPIN_FILE}.\n"
mkdir -p ${NETWORK_DROPIN_DIR}
printf "[Network]\nDNS=${AKSLOCALDNS_NODE_LISTENER_IP}\n\n[DHCP]\nUseDNS=false\n" >${NETWORK_DROPIN_FILE}
