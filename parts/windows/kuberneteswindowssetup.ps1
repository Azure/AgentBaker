...
$global:SkipKubeletConfiguration = [System.Convert]::ToBoolean("{{GetVariable "skipKubeletConfiguration" }}")
$global:KubeletOnly = [System.Convert]::ToBoolean("{{GetVariable "kubeletOnly" }}")
...
try
{
    ...
    # Exit early if the script has been executed (unless running in kubelet-only mode)
    if (Test-Path -Path $CSEResultFilePath -PathType Leaf) {
        if ($global:KubeletOnly) {
            # For KubeletOnly mode, verify that Stage 1 was completed first
            if (-not (Test-Path -Path "C:\AzureData\stage1-complete" -PathType Leaf)) {
                Write-Log "KubeletOnly mode requires Stage 1 to be completed first (stage1-complete marker not found)"
                $global:ExitCode = 1
                $global:ErrorMessage = "Stage 1 must be completed before running KubeletOnly configuration"
                return
            }
            Write-Log "KubeletOnly mode detected - proceeding with kubelet configuration"
        } else {
    ...
    # Function to download and extract CSE scripts package
    function Download-And-Extract-CSEScripts {
        ...
        Expand-Archive $tempfile -DestinationPath $DestinationPath -Force
        Remove-Item -Path $tempfile -Force
    }

    # Handle kubelet-only mode
    if ($global:KubeletOnly) {
        Write-Log "Running in kubelet-only mode..."
        Logs-To-Event -TaskName "AKS.WindowsCSE.KubeletOnlyMode" -TaskMessage "Executing kubelet-only configuration"
        
        # Download and extract CSE scripts to c:\k for kubelet-only mode
        Download-And-Extract-CSEScripts -DestinationPath "c:\k"
        
        # Source the necessary functions
        . c:\k\windowscsehelper.ps1
        . c:\k\kubeletfunc.ps1
        
        # Only run kubelet configuration
        Install-KubernetesServices -KubeDir "c:\k"
    ...
    # Download and extract CSE scripts for normal CSE flow
    Download-And-Extract-CSEScripts -DestinationPath "C:\\AzureData\\windows"
    ...
    if (-not $global:SkipKubeletConfiguration) {
        Logs-To-Event -TaskName "AKS.WindowsCSE.InstallKubernetesServices" -TaskMessage "Installing Kubernetes services"
        Install-KubernetesServices `
            -KubeDir $global:KubeDir
    } else {
        Logs-To-Event -TaskName "AKS.WindowsCSE.SkipKubeletConfiguration" -TaskMessage "Skipping kubelet configuration as requested. Use c:\k\windowskubeletcomplete.ps1 to complete later."
        Write-Log "Skipping kubelet configuration. Kubelet services will need to be installed later using completion script."
    }
...
finally
{
    ...
    # Always create completion marker to satisfy CSE framework validation
    if ($global:ExitCode -ne 0) {
        ...
        if ($global:SkipKubeletConfiguration) {
            # Stage 1: Create marker indicating Stage 2 is needed
            Set-Content -Path "C:\AzureData\stage1-complete" -Value "1" -Force
            Write-Log "Stage 1 complete - kubelet configuration skipped, Stage 2 required"
        }
    }
