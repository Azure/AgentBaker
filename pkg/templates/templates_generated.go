...
- path: /usr/local/bin/configure_masq_rules.sh
  ...
  content: |
    #!/bin/bash
    set -x
    # we are currently enabling ipmasq for kubenet cni with containerd which has the unfortunate
    # side effect of SNATing all outgoing pod-to-pod traffic to node ip
    # kubelet ignores the --non-masquerade-cidr flag for containerd so we will have to handle it ourselves here
    # this iptable rule allows all traffic heading to the cluster vnets to avoid SNAT
    # note this will only run if the cluster does NOT have the ip-masq-agent daemonset addon (currently AKS does not deploy this for kubenet clusters)
