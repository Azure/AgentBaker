...
configGPUDrivers() {
    ...
    if [[ $OS == $UBUNTU_OS_NAME ]]; then
        ...
        if [[ "${CONTAINER_RUNTIME}" == "containerd" ]]; then
            ...
            timeout -k 125s 120s strace -ff -tt bash -c "$CTR_GPU_INSTALL_CMD $NVIDIA_DRIVER_IMAGE:$NVIDIA_DRIVER_IMAGE_TAG gpuinstall /entrypoint.sh install"
            ret=$?
            if [[ "$ret" != "0" ]]; then
...
validateGPUDrivers() {
    ...
    nvidia-modprobe -u -c0 && echo "gpu driver loaded" || configGPUDrivers || exit $ERR_GPU_DRIVERS_START_FAIL
    which nvidia-smi
    if [[ $? == 0 ]]; then
        SMI_RESULT=$(nvidia-smi)
    else
        SMI_RESULT=$($GPU_DEST/bin/nvidia-smi)
    fi
    SMI_STATUS=$?
...
export NVIDIA_DRIVER_IMAGE_SHA="sha-522542"
