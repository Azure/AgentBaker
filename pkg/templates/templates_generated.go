...
- path: /usr/local/bin/configure_masq_rules.sh
  ...
  content: |
    ...
    # kubelet does not handle non-masquerade-cidr for containerd so we will have to handle it ourselves here
    # this iptable rule allows all traffic heading to the cluster vnets to avoid SNAT
    # note this will only run of the cluster does NOT have the ip-masq-agent daemonset addon (currently AKS does not deploy this for kubenet clusters)
