...
if [[ -n "${OUTBOUND_COMMAND}" ]]; then
    logs_to_events "AKS.CSE.outboundCommand" "retrycmd_if_failure 50 1 5 ${OUTBOUND_COMMAND}" >> /var/log/azure/cluster-provision-cse-output.log 2>&1 || exit $ERR_OUTBOUND_CONN_FAIL;
fi
