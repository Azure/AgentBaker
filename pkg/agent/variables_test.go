...
var _ = Describe("Windows custom data variables check", func() {
	...
	It("sets serviceAccountImagePullBindingEnabled to true", func() {
		config.ContainerService.Properties.SecurityProfile = &datamodel.SecurityProfile{
			ServiceAccountImagePullProfile: &datamodel.ServiceAccountImagePullProfile{
				Enabled: true,
			},
		}
		vars := getWindowsCustomDataVariables(config)
	...
	It("sets serviceAccountImagePullBindingEnabled to false when SecurityProfile is nil", func() {
		config.ContainerService.Properties.SecurityProfile = nil
		vars := getWindowsCustomDataVariables(config)
		Expect(vars["serviceAccountImagePullBindingEnabled"]).To(Equal("false"))
	})
	...
	It("sets serviceAccountImagePullDefaultClientID to empty when SecurityProfile is nil", func() {
		config.ContainerService.Properties.SecurityProfile = nil
		vars := getWindowsCustomDataVariables(config)
	...
	It("sets serviceAccountImagePullDefaultTenantID to empty when SecurityProfile is nil", func() {
		config.ContainerService.Properties.SecurityProfile = nil
		vars := getWindowsCustomDataVariables(config)
	...
	It("sets identityBindingsLocalAuthoritySNI to empty when SecurityProfile is nil", func() {
		config.ContainerService.Properties.SecurityProfile = nil
		vars := getWindowsCustomDataVariables(config)
