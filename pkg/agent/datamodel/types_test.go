...
func TestGetCriticalHostsEntriesContent(t *testing.T) {
	tests := []struct {
		name             string
		agentPoolProfile *AgentPoolProfile
		expected         string
		...
		},
		{
			...
			agentPoolProfile: &AgentPoolProfile{
				LocalDNSProfile: &LocalDNSProfile{
					CriticalHostsEntries: map[string][]string{
						"mcr.microsoft.com": {"20.61.99.68"},
					},
				},
			},
			expected: "# AKS critical FQDN addresses provided by AKS-RP\n" +
				"# This file is written by CSE during node provisioning\n\n" +
				"# mcr.microsoft.com\n" +
				"20.61.99.68 mcr.microsoft.com\n\n",
		},
		{
			name: "single FQDN with multiple IPs (IPv4 and IPv6)",
			agentPoolProfile: &AgentPoolProfile{
				LocalDNSProfile: &LocalDNSProfile{
					CriticalHostsEntries: map[string][]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var actual string
			if tt.agentPoolProfile != nil {
				actual = tt.agentPoolProfile.GetCriticalHostsEntriesContent()
			}
