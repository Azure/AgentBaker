...
func TestGetMinMaxVersion(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			t.Parallel()
			theMin := GetMinVersion(c.versions, c.preRelease)
			if theMin != c.expectedMin {
				t.Errorf("GetMinVersion returned the wrong min version, expected %s, got %s", c.expectedMin, theMin)
			}
			theMax := GetMaxVersion(c.versions, c.preRelease)
			if theMax != c.expectedMax {
				t.Errorf("GetMaxVersion returned the wrong max version, expected %s, got %s", c.expectedMax, theMax)
			}
		})
	}
}
