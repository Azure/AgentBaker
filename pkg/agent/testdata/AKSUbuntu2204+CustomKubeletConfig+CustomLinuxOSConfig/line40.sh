...
pullContainerImage() {
    ...
    if [[ ${CLI_TOOL} == "ctr" ]]; then 
        logs_to_events "AKS.CSE.imagepullctr.${CONTAINER_IMAGE_URL}" "retrycmd_if_failure 2 1 120 ctr --namespace k8s.io image pull $CONTAINER_IMAGE_URL" || {
            echo "timed out pulling image ${CONTAINER_IMAGE_URL} via ctr"
            exit $ERR_CONTAINERD_CTR_IMG_PULL_TIMEOUT
        }
    elif [[ ${CLI_TOOL} == "crictl" ]]; then 
        logs_to_events "AKS.CSE.imagepullcrictl.${CONTAINER_IMAGE_URL}" "retrycmd_if_failure 2 1 120 crictl pull $CONTAINER_IMAGE_URL" || {
            echo "timed out pulling image ${CONTAINER_IMAGE_URL} via crictl"
            exit $ERR_CONTAINERD_CRICTL_IMG_PULL_TIMEOUT
        }
    else
        logs_to_events "AKS.CSE.imagepull.${CONTAINER_IMAGE_URL}" "retrycmd_if_failure 2 1 120 docker pull $CONTAINER_IMAGE_URL" || {
            echo "timed out pulling image ${CONTAINER_IMAGE_URL} via docker"
            exit $ERR_DOCKER_IMG_PULL_TIMEOUT
        }
    fi
}
