...
SORTED_RELEASE=""
sortEtcRelease() {
    SORTED_RELEASE=$(sort -r /etc/*-release)
}

OS=$($SORTED_RELEASE | gawk 'match($0, /^(ID_LIKE=(coreos)|ID=(.*))$/, a) { print toupper(a[2] a[3]); exit }')
OS_VERSION=$($SORTED_RELEASE | gawk 'match($0, /^(VERSION_ID=(.*))$/, a) { print toupper(a[2] a[3]); exit }' | tr -d '"')
