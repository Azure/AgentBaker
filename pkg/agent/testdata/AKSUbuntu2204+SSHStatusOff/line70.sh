...
configGPUDrivers() {
    ...
    logs_to_events "AKS.CSE.configGPUDrivers.nvidia-smi" retrycmd_if_failure 120 5 300 nvidia-smi || exit $ERR_GPU_DRIVERS_START_FAIL
...
validateGPUDrivers() {
    ...
    which nvidia-smi
    if [[ $? == 0 ]]; then
        logs_to_events "AKS.CSE.validateGPUDrivers.nvidia-smi" SMI_RESULT=$(retrycmd_if_failure 24 5 300 nvidia-smi)
    else
        logs_to_events "AKS.CSE.validateGPUDrivers.nvidia-smi" SMI_RESULT=$(retrycmd_if_failure 24 5 300 $GPU_DEST/bin/nvidia-smi)
    fi
    SMI_STATUS=$?
