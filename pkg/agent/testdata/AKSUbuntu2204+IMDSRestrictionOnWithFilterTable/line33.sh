...
if [ "${IS_KRUSTLET}" == "true" ]; then
    ...
    logs_to_events "AKS.CSE.logDownloadURL" downloadURLWasm=$(update_base_url $downloadURLWasm)
    ...
    logs_to_events "AKS.CSE.logDownloadURL" downloadURLSpinKube=$(update_base_url $downloadURLSpinKube)
