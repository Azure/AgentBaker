...
func TestUnmarshalConfigurationV1(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// For the test file case, just check that we got a valid config
			...
			if tt.want != nil && got != nil {
				if got.Version != tt.want.Version {
					t.Errorf("UnmarshalConfigurationV1() version = %v, want %v", got.Version, tt.want.Version)
				}
				if tt.want.AuthConfig != nil && got.AuthConfig != nil {
					if got.AuthConfig.SubscriptionId != tt.want.AuthConfig.SubscriptionId {
						t.Errorf("UnmarshalConfigurationV1() subscriptionId = %v, want %v",
							got.AuthConfig.SubscriptionId, tt.want.AuthConfig.SubscriptionId)
					}
				}
			}
		})
