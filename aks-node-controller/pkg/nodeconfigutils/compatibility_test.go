...
func setupWorktree(t *testing.T, targetCommit string) string {
	// CI checks out a shallow clone, so the historical commit may not be present
	// locally. Make sure it is fetched (with retries) before creating the worktree,
	// otherwise `git worktree add` fails with "invalid reference" and the test flakes.
	ensureCommitAvailable(t, targetCommit)
...
// Best-effort only: per-commit availability is enforced by ensureCommitAvailable before use.
...
// fetch to absorb transient network failures (blips, rate limiting) that would
// otherwise leave the commit missing and make `git worktree add` flaky in CI.
func ensureCommitAvailable(t *testing.T, commit string) {
	t.Helper()
	const maxAttempts = 5
	...
	require.Truef(t, commitExists(commit),
		"commit %s is not available locally after %d fetch attempts; "+
			"ensure the CI checkout has access to repository history (e.g. fetch-depth: 0)",
		commit, maxAttempts)
}
