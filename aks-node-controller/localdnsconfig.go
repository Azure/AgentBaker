...
func (a *App) fetchLocalDNSConfigFromLPS(ctx context.Context) (string, error) {
	...
	if err != nil {
		if statusCode, ok := localDNSLPSUnavailableStatusCode(status.Code(err)); ok {
			return "", &lpsUnavailableError{statusCode: statusCode}
		}
...
func localDNSLPSUnavailableStatusCode(code codes.Code) (int, bool) {
	switch code {
	case codes.NotFound:
