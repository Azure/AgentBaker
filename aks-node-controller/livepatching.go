...
// live-patching is a generic, component-agnostic client for the live-patching service.
//
...
// worse than missing a hotfix, so it must never fail. live-patching is an operator/diagnostic and
...
func (a *App) runLivePatchingCommand(ctx context.Context, out io.Writer, component string) error {
	...
	if component == "" {
		return fmt.Errorf("live-patching requires a component argument")
	}

	slog.Info("aks-node-controller live-patching started", "component", component)

	config, err := a.fetchComponentConfig(ctx, component)
	if err != nil {
		slog.Error("aks-node-controller live-patching failed", "component", component, "error", err)
	...
	slog.Info("aks-node-controller live-patching finished", "component", component, "bytes", len(config))
	return nil
}
...
func (a *App) fetchComponentConfig(ctx context.Context, component string) ([]byte, error) {
	...
	return a.fetchComponentConfigOverGRPC(ctx, component, "live-patching")
}
