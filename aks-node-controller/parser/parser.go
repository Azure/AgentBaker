...
func getCSEEnv(ctx context.Context, config *aksnodeconfigv1.Configuration) map[string]string {
	...
	env := map[string]string{
		...
		"MIG_NODE":                                             fmt.Sprintf("%v", getIsMIGNode(config.GetGpuConfig().GetGpuInstanceProfile(), config.GetGpuConfig().GetMigProfiles())),
		...
		"NVIDIA_MIG_PROFILES":                                  getStringifiedStringArray(config.GetGpuConfig().GetMigProfiles(), ","),
		...
		"SECURE_TLS_BOOTSTRAPPING_VALIDATE_KUBECONFIG_TIMEOUT": config.GetBootstrappingConfig().GetSecureTlsBootstrappingValidateKubeconfigTimeout(),
		"SECURE_TLS_BOOTSTRAPPING_GET_ACCESS_TOKEN_TIMEOUT":    config.GetBootstrappingConfig().GetSecureTlsBootstrappingGetAccessTokenTimeout(),
		"SECURE_TLS_BOOTSTRAPPING_GET_INSTANCE_DATA_TIMEOUT":   config.GetBootstrappingConfig().GetSecureTlsBootstrappingGetInstanceDataTimeout(),
