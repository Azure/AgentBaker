...
func TestDownloadHotfix_AppliesScriptHotfixWriteFiles(t *testing.T) {
	origVersion := Version
	Version = "202604.01.1"
	defer func() { Version = origVersion }()

	dir := t.TempDir()
	hotfixConfigPath := filepath.Join(dir, "hotfix-config.json")
	scriptHotfixPath := filepath.Join(dir, "cse_install.sh")
	...
	require.NoError(t, os.WriteFile(nodeCustomDataPath, []byte(
		`#cloud-config
write_files:
- path: `+hotfixConfigPath+`
  permissions: "0644"
  owner: root
  content: |
    {"target_version":"202604.01"}
- path: `+normalPath+`
  permissions: "0644"
  owner: root
  content: |
...
`), 0o644))
	...
	tt := NewTestApp(t, TestAppConfig{})
	tt.App.hotfixVersionPath = hotfixConfigPath
	tt.App.nodeCustomDataPath = nodeCustomDataPath
	require.NoError(t, tt.App.downloadHotfix(context.Background()))

	// download-hotfix applies only script hotfix files, not normal write_files.
	_, err := os.Stat(normalPath)
	assert.True(t, os.IsNotExist(err))

	hotfixScript, err := os.ReadFile(scriptHotfixPath)
	require.NoError(t, err)
	assert.Equal(t, "hotfix-script\n", string(hotfixScript))
}
