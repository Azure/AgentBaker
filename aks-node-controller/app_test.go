...
func TestApp_Run(t *testing.T) {
	...
	t.Run("provision command with provision-config and nbc-cmd flag", func(t *testing.T) {
		tt := NewTestApp(t, TestAppConfig{})
		exitCode := tt.App.Run(context.Background(), []string{"aks-node-controller", "provision", "--provision-config=parser/testdata/test_aksnodeconfig.json", "--nbc-cmd=parser/testdata/test_nbccmd.sh"})
		assert.Equal(t, 0, exitCode)
