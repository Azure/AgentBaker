...
func TestLivePatchingCommand_ExitCodes(t *testing.T) {
	t.Run("missing component argument", func(t *testing.T) {
		tt := NewTestApp(t, TestAppConfig{})
		exitCode := tt.App.Run(context.Background(), []string{"aks-node-controller", "live-patching"})
	...
	t.Run("too many arguments", func(t *testing.T) {
		...
		exitCode := tt.App.Run(context.Background(),
			[]string{"aks-node-controller", "live-patching", "securityPatch", "extra"})
		assert.Equal(t, 1, exitCode)
	})
	...
	t.Run("fetch failure is not fail-open", func(t *testing.T) {
		...
		exitCode := tt.App.Run(context.Background(),
			[]string{"aks-node-controller", "live-patching", "securityPatch"})
		assert.Equal(t, 1, exitCode)
	})
...
func TestFetchComponentConfigOverGRPC_GenericComponent(t *testing.T) {
	...
	got, err := tt.App.fetchComponentConfigOverGRPC(context.Background(), "securityPatch", "live-patching")
