...
type App struct {
	...
	// by the live-patching command, letting unit tests inject a canned config or error without
	// real networking.
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				Action: func(ctx context.Context, cmd *cli.Command) error {
					args := cmd.Args().Slice()
					if len(args) != 1 {
						return fmt.Errorf("live-patching requires exactly one component argument, got %d: %s",
