...
| `DisablePubkeyAuth` | `bool` | Specifies whether to disable ssh public key authentication on the VM node. This is labeled as optional (explicit presence) so that we know whether it's set or not. If it's not set, the default value will be nil, but will be set to false on the VHD. That is, the default behavior is to enable ssh public key authentication.                                               | `TURN_OFF_PUBKEY_AUTH_SSH` |
