...
- **provision-wait**: waits for a provisioning completion marker to appear and reads `provision.json` which contains the provision output of type `CSEStatus` and is returned by CSE through capturing stdout.
    - The command is byte-identical for a normal node and for a pre-provision (PIS image bake) run. It watches two markers and reports on whichever appears:
      ...
      Both marker directories are watched **before** either is stat-ed, so a marker created in between is still observed.
    ...
    - On a fail-fast error the controller writes `provision.complete` even during a bake. The recorded result carries a non-zero `ExitCode`, so `provision-wait` fails, the bake fails and no image is captured. It also overwrites any existing `provision.json` rather than preserving it, so a stale success cannot be reported as this run's result. The cloud-init boothook additionally deletes a leftover `provision.json` once it has passed the `provision.complete` early-exit guard.
