...
- **provision-wait**: waits for a provisioning completion marker to appear and reads `provision.json` which contains the provision output of type `CSEStatus` and is returned by CSE through capturing stdout.
    - The command is byte-identical for a normal node and for a pre-provision (PIS image bake) run. It watches two markers and reports on whichever appears:
        - `/opt/azure/containers/provision.complete` — durable, written by a normal provisioning run.
        - `/run/azure/pre-provision.complete` — volatile, written when `PreProvisionOnly` is set. `/run` is a tmpfs cleared on every boot, so it cannot be captured into the image.
    - Both marker directories are watched **before** either is stat-ed, so a marker created in between is still observed. The durable marker's directory is required; the volatile one is best effort, so an unusable `/run` is logged and skipped rather than breaking every normal node (a bake then fails loudly by timeout).
    - A bake runs `basePrep` only, then writes `base_prep.complete` (durable phase state, deliberately captured into the image so a node created from it skips `basePrep`) plus the volatile marker. It never writes `provision.complete`: that file is durable, and a captured copy would make `cse_main.sh` exit before `nodePrep` on every node created from the image.
    - A bake is only a bake if `PRE_PROVISION_ONLY` reaches the CSE command. With `--nbc-cmd` it comes from the rendered NBC command; with `--provision-config` the parser renders it from `Configuration.PreProvisionOnly`, so the caller building that config must set the field.
    - Because the bake's completion signal is volatile, a node created from the image cannot inherit it. This does not rely on the image pipeline deleting anything.
    - On a fail-fast error the controller writes `provision.complete` even during a bake. The recorded result carries a non-zero `ExitCode`, so `provision-wait` fails, the bake fails and no image is captured.
    - Before each provisioning attempt the controller clears a `provision.json` left from an earlier run, unless `provision.complete` shows this node already finished provisioning. A node created from a PIS image can otherwise inherit the bake's result file and report that inherited success. If the stale file cannot be removed the controller fails closed: it replaces the file with the current failure and writes a marker. If that replacement also fails it writes **no** marker, so `provision-wait` and CSE time out rather than reporting the stale result. Once the reset has succeeded, a fail-fast error only writes `provision.json` when absent, so the richer result `cse_start` produces is preserved and never truncated while `provision-wait` is reading it.
