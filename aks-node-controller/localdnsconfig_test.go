...
func TestLocalDNSLPSUnavailableStatusCode(t *testing.T) {
	tests := []struct {
		name       string
		code       codes.Code
		statusCode int
		...
		{name: "not found", code: codes.NotFound, statusCode: http.StatusNotFound, ok: true},
		{name: "permission denied", code: codes.PermissionDenied, statusCode: http.StatusForbidden, ok: true},
		{name: "unauthenticated", code: codes.Unauthenticated, statusCode: http.StatusUnauthorized, ok: true},
		{name: "internal", code: codes.Internal, ok: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			statusCode, ok := localDNSLPSUnavailableStatusCode(tt.code)
