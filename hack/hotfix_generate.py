#!/usr/bin/env python3
"""
Detects changed provisioning scripts and writes the corresponding write_files
entries to the script-hotfix payload file (parts/hotfix/anc-scripts-hotfix-files.yml).
...
The payload file is a standalone #cloud-config document that baker ships as its own
boothook file. aks-node-controller's `download-hotfix` subcommand applies the write_files
entries when the node's scripts_version matches. When no provisioning scripts changed, the
payload is reset to an empty placeholder (no "- path:" entries), which baker ships nothing
for and download-hotfix treats as a no-op.

The write_files block definitions are sourced from the traditional (non-scriptless) section
of nodecustomdata.yml so the two stay in sync; nodecustomdata.yml itself is never modified.

Note: once we move to the final stage of scriptless, we no longer need to hotfix the scripts
and can remove this script entirely.
"""
...
OUTPUT = "parts/hotfix/anc-scripts-hotfix-files.yml"
ARTIFACTS_DIR = "parts/linux/cloud-init/artifacts"

# Default placeholder content written when there is no active script hotfix. An empty
# write_files list (no "- path:" entries) signals baker to ship nothing and makes
# download-hotfix a no-op. Keep this in sync with the committed default of OUTPUT.
DEFAULT_PAYLOAD = (
    "#cloud-config\n"
    "# Script hotfix write_files payload. Auto-generated by hack/hotfix_generate.py when\n"
    "# provisioning scripts change. An empty write_files list (no \"- path:\" entries) means no\n"
    "# active script hotfix, in which case baker ships nothing and download-hotfix is a no-op.\n"
    "write_files:\n"
)

# Header prepended to the payload when one or more write_files blocks are emitted.
PAYLOAD_HEADER = (
    "#cloud-config\n"
    "# Script hotfix write_files payload. Auto-generated by hack/hotfix_generate.py.\n"
    "# Do not edit by hand. Sourced from the traditional section of nodecustomdata.yml.\n"
    "# aks-node-controller's download-hotfix applies these entries when scripts_version matches.\n"
    "write_files:\n"
)
...
def reset_payload():
    """Reset the payload file to the empty placeholder (no active script hotfix)."""
    with open(OUTPUT, 'w') as f:
        f.write(DEFAULT_PAYLOAD)
    print(f"Reset {OUTPUT} to empty placeholder (no active script hotfix).", file=sys.stderr)


def write_payload(target_varkeys):
    """Extract matching write_files blocks from the traditional section of nodecustomdata.yml
    and write them as a standalone #cloud-config payload to OUTPUT.

    Returns True if one or more blocks were written, False if no matching blocks were found.
    """
    ...
    payload_lines = [PAYLOAD_HEADER]
    for block_lines in selected_blocks:
        payload_lines.extend(block_lines)
    ...
    print(f"\nWrote {len(selected_blocks)} write_files block(s) to {OUTPUT}", file=sys.stderr)
    return True
...
def main():
    ...
    changed = write_payload(target_varkeys)
    if changed:
        print("\nDone. Payload updated.")
    else:
        reset_payload()
        print("\nNo matching blocks; payload reset to placeholder.")
