...
# hotfix-generate.sh
#
# Detects changed provisioning scripts and updates hotfix.json with the
# corresponding variable property keys. The Go template rendering pipeline
# reads hotfix.json to inject write_files entries into the
...
#
# This script is called by the hotfix-generate GH Action, or can be run
# manually by a developer before running 'make generate'.
#
# Note: once we move to the final stage of scriptless, we no longer need to
# hotfix the scripts into the template and can remove this script entirely.

set -euo pipefail

HOTFIX_CONFIG="parts/linux/cloud-init/hotfix.json"
...
# When any variant in a group changes, all members of the group should be included
# so the template renders correctly for all distros.
...
echo ""
echo "Variable keys to hotfix: ${!matched_varkeys[*]}"

# Write hotfix.json
json_array="["
first=true
for key in "${!matched_varkeys[@]}"; do
    if [[ "${first}" == "true" ]]; then
        first=false
    else
        json_array+=","
...
echo "${json_array}" > "${HOTFIX_CONFIG}"
echo ""
echo "Updated ${HOTFIX_CONFIG}:"
cat "${HOTFIX_CONFIG}"
echo ""
