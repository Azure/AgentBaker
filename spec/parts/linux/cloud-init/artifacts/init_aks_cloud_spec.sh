...
EOF
    ...
    It 'passes refresh_location into determine_cert_endpoint_mode and emits legacy mode event'
        When run env -u __SOURCED__ PATH="${MOCK_BIN_DIR}:$PATH" LOCATION="zzzz" bash "${SCRIPT_COPY}" ca-refresh usseceast
        The stdout should include 'Using custom cloud certificate endpoint mode: legacy'
