...
Describe 'cse_helpers.sh'
    ...
    Describe 'configureSSHService'
        setup() {
            # Create a test directory structure for systemd files
            mkdir -p /tmp/etcsystemd/system/ssh.service.d
            mkdir -p /tmp/etcsystemd/system/ssh.socket.d
            ...
            # Override etc_systemd path for testing
        ...
        It 'should do nothing for Ubuntu 22.04 (supported version below 22.10)'
            # Mock grep to return Ubuntu 22.04
            grep() {
                echo "DISTRIB_RELEASE=22.04"
            }
            
            When call configureSSHService
            The output should include "SSH Service configuration not needed for Ubuntu version 22.04 (< 22.10)"
            The status should be success
        End
        
        It 'should configure SSH service for Ubuntu 24.04 (supported version above 22.10)'
            # Mock grep to return Ubuntu 24.04
            grep() {
                echo "DISTRIB_RELEASE=24.04"
            }
            
            When call configureSSHService
            The output should include "Configuring SSH service for Ubuntu 24.04"
            The output should include "Disabling ssh.socket"
            The output should include "Removing /tmp/etcsystemd/system/ssh.service.d/00-socket.conf"
            The output should include "Removing /tmp/etcsystemd/system/ssh.socket.d/addresses.conf"
            The output should include "Enabling ssh"
            The output should include "Starting ssh"
            The output should include "SSH Service configured successfully"
            The status should be success
        End
        ...
        It 'should handle missing configuration files gracefully for Ubuntu 24.04'
            ...
            The output should include "Configuring SSH service for Ubuntu 24.04"
            The output should include "Disabling ssh.socket"
            The output should not include "Removing /tmp/etcsystemd/system/ssh.service.d/00-socket.conf"
            The output should not include "Removing /tmp/etcsystemd/system/ssh.socket.d/addresses.conf"
            The output should include "Enabling ssh"
            The output should include "Starting ssh"
            The output should include "SSH Service configured successfully"
            The status should be success
        ...
        It 'should handle ssh.socket already being disabled for Ubuntu 24.04'
            ...
            # Override systemctl to simulate socket already being disabled
            systemctl() {
                case "$1" in
                    "disable")
                        if [ "$2" = "ssh.socket" ]; then
                            echo "ssh.socket already disabled"
                            return 1
                        else
                            echo "Disabling $2"
                            return 0
            ...
            When call configureSSHService
            The output should include "Configuring SSH service for Ubuntu 24.04"
            The output should include "ssh.socket already disabled"
            The output should include "Removing /tmp/etcsystemd/system/ssh.service.d/00-socket.conf"
            The output should include "Removing /tmp/etcsystemd/system/ssh.socket.d/addresses.conf"
            The output should include "Enabling ssh"
            The output should include "Starting ssh"
