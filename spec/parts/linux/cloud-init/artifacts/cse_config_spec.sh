...
OVERRIDE_EOF
    ...
    Describe 'configGPUDrivers'
        ...
        It 'times the image pull and install steps on Ubuntu'
            ...
            The status should be success
            The output line 1 should equal "logs_to_events AKS.CSE.configGPUDrivers.configureNvidiaCDIRefresh"
            The output line 2 should equal "logs_to_events AKS.CSE.configGPUDrivers.pullGPUDriverImage"
            ...
            The output line 7 should equal "logs_to_events AKS.CSE.configGPUDrivers.repairNvidiaCDIRefresh"
        End
        ...
        It 'warns but still starts containerd when CDI repair fails on Ubuntu'
            OS="UBUNTU"
            isMarinerOrAzureLinux() { return 1; }
            isAzureLinuxOSGuard() { return 1; }
            isACL() { return 1; }
            logs_to_events() {
                if [ "$1" = "AKS.CSE.configGPUDrivers.repairNvidiaCDIRefresh" ]; then
                    return 1
                fi
                return 0
            }
            ...
            # CDI is not on the GPU-pod data path, so a failed repair must not fail provisioning.
            The status should be success
            The output should include "NVIDIA CDI refresh repair failed"
            The output should include "CONTAINERD_STARTED"
        End
    ...
    Describe 'repairNvidiaCDIRefresh'
        MIG_NODE="false"

        It 'resets the failed units and verifies a freshly generated NVIDIA CDI device'
            SYSTEMD_UNIT_DIR="$(mktemp -d)"
            NVIDIA_CDI_SPEC_PATH="${SYSTEMD_UNIT_DIR}/run/cdi/nvidia.yaml"
            mkdir -p "$(dirname "$NVIDIA_CDI_SPEC_PATH")"
            ...
            When call repairNvidiaCDIRefresh
            ...
            The output line 4 should equal "systemctl start nvidia-cdi-refresh.service"
            The output line 5 should equal "systemctl start nvidia-cdi-refresh.path"
            The contents of file "$NVIDIA_CDI_SPEC_PATH" should equal "fresh"
            rm -rf "${SYSTEMD_UNIT_DIR}"
        End

        It 'does nothing when the toolkit has no CDI units'
            systemctl() {
                if [ "$1" = "show" ]; then
                    echo "not-found"
                    return 0
        ...
        It 'fails when only one CDI unit exists'
            systemctl() {
                if [ "$1" = "show" ] && [ "$4" = "nvidia-cdi-refresh.service" ]; then
                    echo "loaded"
                elif [ "$1" = "show" ]; then
                    echo "not-found"
                fi
        ...
        It 'fails before starting the service when reset-failed fails'
            systemctl() {
                if [ "$1" = "show" ]; then
                    echo "loaded"
                    return 0
        ...
        It 'rearms the units without verifying on a MIG node'
            MIG_NODE="true"
            SYSTEMD_UNIT_DIR="$(mktemp -d)"
            NVIDIA_CDI_SPEC_PATH="${SYSTEMD_UNIT_DIR}/run/cdi/nvidia.yaml"
            systemctl() {
            ...
            # nvidia-ctk exits 1 until the MIG reboot creates the instances, and ensureMigPartition
            # runs after this, so the spec cannot exist yet. Re-arm and defer to the .path unit.
            The status should be success
            The output line 1 should equal "systemctl stop nvidia-cdi-refresh.path"
            The output line 2 should equal "systemctl stop nvidia-cdi-refresh.service"
            The output line 3 should equal "systemctl reset-failed nvidia-cdi-refresh.service nvidia-cdi-refresh.path"
